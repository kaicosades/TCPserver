...
BOOL CALLBACK DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		//ограниечение prefix, переназначение кнопок
		SendMessage(hSpinPrefix, UDM_SETRANGE, 0, MAKELPARAM(32, 0)); //edm - up/down message
	}
	break;
	case WM_KILLFOCUS:
	{
	...
	case WM_NOTIFY:
	{
		...
		case IDC_IPADDRESS:
			...
			//if ((HWND)wParam == hIPaddress)
			{
				...
				DWORD first = FIRST_IPADDRESS(dwIPaddress);
				DWORD dwMask = 0;
				if (first < 128)dwMask = 0xFF000000; //f - 4 единицы
		...
		case IDC_IPMASK:
		{
			...
			int i = 32;
			for (; dwIPmask & 0x01 ^ 1; i--)dwIPmask >>=1;//dwIPmask >>=1 - сдвигает маску на 1 бит вправо. Условие это побитовые оператор, который проверяет, установлен бит или нет
			CHAR prefix[5];
			sprintf(prefix, "%i", i);
	...
	case WM_COMMAND:
		...
		case IDC_EDIT_PREFIX:
		{
			...
			{
				HWND hEditPrefix = GetDlgItem(hwnd, IDC_EDIT_PREFIX);
				CONST INT SIZE = 3; //колличесво символов
				CHAR sz_prefix[SIZE] = {};
...
VOID GetInfo(HWND hwnd)
{
	...
	DWORD dwBroadcast = dwIPaddress | ~dwIPmask;//широковещательный адрес
...
VOID GetIPBytes(DWORD dwIPaddress, CHAR szIPaddress[])
{
	...
	for (int i = 4 - 1; i   >= 0; i--)
	{
		...
		dwIPaddress >>=  8;//смещение на 8 бит вправо
	...
	*strrchr(szIPaddress, '.') = 0; // разыменовываем возвращаемый функцией указатель и удаляем последнюю точку
	//return szIPaddress;
}